...
class SEALOGBLDataset(Dataset):
    def __init__(
        ...
        if 'Graphormer' in args.model:
            ...
            if directed:
                A_undirected = ssp.csr_matrix((np.concatenate([edge_weight, edge_weight]), (
                np.concatenate([data_pyg.edge_index[0], data_pyg.edge_index[1]]),
                np.concatenate([data_pyg.edge_index[1], data_pyg.edge_index[0]]))),
                                              shape=(data_pyg.num_nodes, data_pyg.num_nodes))
...
def test(eval_metric):
    ...
    result = eval_model(pos_val_pred=pos_val_pred, neg_val_pred=neg_val_pred, pos_test_pred=pos_test_pred,
                        neg_test_pred=neg_test_pred,
                        val_pred=val_pred, val_true=val_true, test_pred=test_pred, test_true=test_true,
                        eval_metric=eval_metric)
    if eval_metric != 'auc':
        result_auc = eval_model(pos_val_pred=pos_val_pred, neg_val_pred=neg_val_pred, pos_test_pred=pos_test_pred,
                                neg_test_pred=neg_test_pred,
                                val_pred=val_pred, val_true=val_true, test_pred=test_pred, test_true=test_true,
                                eval_metric='auc')
...
def test_multiple_models_origin(models, eval_metric):
    ...
    print(
        f' after val - {mem.percent:7} - {mem.free / 1024 ** 3:12.2f} - {mem.available / 1024 ** 3:13.2f} - {mem.used / 1024 ** 3:12.2f}')
    ...
    mem = psutil.virtual_memory()
    print(
        f' after test - {mem.percent:7} - {mem.free / 1024 ** 3:12.2f} - {mem.available / 1024 ** 3:13.2f} - {mem.used / 1024 ** 3:12.2f}')
    results = eval_multiple_models(num_models,
                                   ...
                                   val_preds=val_preds, val_trues=val_trues, test_preds=test_preds,
                                   test_trues=test_trues, eval_metric=eval_metric)
    if eval_metric != 'auc':
        results_auc = eval_multiple_models(num_models,
                                           pos_val_preds=pos_val_preds, neg_val_preds=neg_val_preds,
                                           pos_test_preds=pos_test_preds, neg_test_preds=neg_test_preds,
                                           val_preds=val_preds, val_trues=val_trues, test_preds=test_preds,
                                           test_trues=test_trues, eval_metric='auc')
...
def test_multiple_models(models, loader, num_datas):
    ...
    y_preds, y_trues = [torch.zeros([num_datas]) for _ in range(num_models)], [torch.zeros([num_datas]) for _ in
                                                                               range(num_models)]
    start = 0
    for data in tqdm(loader, ncols=70):
        data = data.to(device)
    ...
    mem = psutil.virtual_memory()
    print(
        f'       max - {mem.percent:7} - {mem.free / 1024 ** 3:12.2f} - {mem.available / 1024 ** 3:13.2f} - {mem.used / 1024 ** 3:12.2f}')
...
parser.add_argument('--trackback', action='store_true',
                    help="whether to enabale trackback path searching in random walk")
...
if not args.keep_old:
    ...
    for sub_dir in ['', 'surel_gacc']:
        ...
        files = [f for f in os.listdir(full_dir) if
                 os.path.isfile(os.path.join(full_dir, f)) and os.path.splitext(f)[1] in ['.py", ".c", ".cpp']]
...
if args.use_heuristic:
    ...
    for idx_type in range(len(cn_types)):
        ...
        if args.eval_metric == 'hits':
            results = evaluate_hits(pos_val_pred[idx_type], neg_val_pred[idx_type], pos_test_pred[idx_type],
                                    neg_test_pred[idx_type])
        elif args.eval_metric == 'mrr':
            results = evaluate_mrr(pos_val_pred[idx_type], neg_val_pred[idx_type], pos_test_pred[idx_type],
                                   neg_test_pred[idx_type])
        elif args.eval_metric == 'auc':
...
for run in range(args.runs):
    if args.model == 'DGCNN':
        ...
        model = GCNGraphormer(args, args.hidden_channels, args.num_layers, args.max_z, train_dataset,
                              use_feature=args.use_feature, use_feature_GT=args.use_feature_GT, node_embedding=emb).to(
            device)
        ...
        model = GCNGraphormer_noNeigFeat(args, args.hidden_channels, args.num_layers, args.max_z, train_dataset,
                                         use_feature=args.use_feature, use_feature_GT=args.use_feature_GT,
                                         node_embedding=emb, z_emb_agg=z_emb_agg).to(device)
        ...
        model = SingleGraphormer(args, args.hidden_channels, args.num_layers, args.max_z,
                                 train_dataset=train_dataset, use_feature=args.use_feature,
    ...
    if args.continue_from is not None:
        model.load_state_dict(
            torch.load(os.path.join(args.res_dir,
                                    'run{}_model_checkpoint{}.pth'.format(args.continue_from[0],
                                                                          args.continue_from[-1])))
        )
        if not args.only_final_test:
            optimizer.load_state_dict(
                torch.load(os.path.join(args.res_dir,
                                        'run{}_optimizer_checkpoint{}.pth'.format(args.continue_from[0],
                                                                                  args.continue_from[-1])))
            )
    ...
    if args.part_continue_from is not None:  # stage2 training
        model.ngnndgcnn.load_state_dict(
            torch.load(os.path.join(args.res_dir,
                                    'run{}_model_checkpoint{}.pth'.format(args.part_continue_from[0],
                                                                          args.part_continue_from[-1])))
        )
    ...
    if len(args.test_multiple_models) > 0:
        ...
        mem = psutil.virtual_memory()
        print(
            f'     begin - {mem.percent:7} - {mem.free / 1024 ** 3:12.2f} - {mem.available / 1024 ** 3:13.2f} - {mem.used / 1024 ** 3:12.2f}')
        if args.eval_metric != 'auc':
            val_preds, val_trues, pos_val_preds, neg_val_preds = test_multiple_models(models, val_loader,
                                                                                      len(val_dataset))
        else:
        ...
        print(
            f' after val - {mem.percent:7} - {mem.free / 1024 ** 3:12.2f} - {mem.available / 1024 ** 3:13.2f} - {mem.used / 1024 ** 3:12.2f}')
        if args.eval_metric != 'auc':
            test_preds, test_trues, pos_test_preds, neg_test_preds = test_multiple_models(models, test_loader,
                                                                                          len(test_dataset))
        else:
        ...
        results = eval_multiple_models(num_models,
                                       ...
                                       pos_test_preds=pos_test_preds, neg_test_preds=neg_test_preds,
                                       val_preds=val_preds, val_trues=val_trues, test_preds=test_preds,
                                       test_trues=test_trues, eval_metric=args.eval_metric)
        if args.eval_metric != 'auc':
            results_auc = eval_multiple_models(num_models,
                                               pos_val_preds=pos_val_preds, neg_val_preds=neg_val_preds,
                                               pos_test_preds=pos_test_preds, neg_test_preds=neg_test_preds,
        ...
        mem = psutil.virtual_memory()
        print(
            f'     final - {mem.percent:7} - {mem.free / 1024 ** 3:12.2f} - {mem.available / 1024 ** 3:13.2f} - {mem.used / 1024 ** 3:12.2f}')
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):
        ...
        if epoch % args.eval_steps == 0:
            results = test(
                args.eval_metric)  # {'MRR': (0.7427010536193848, 0.7336037158966064), 'AUC': (0.9981022174148187, 0.9458885884261763)}
    ...
    if len(loggers.keys()) > 1:
        ...
        candidate_epochs = [i + 1 for i in candidate_idxs] if args.part_continue_from is None else [i + 1001 for i in
                                                                                                    candidate_idxs]
        print(
            f'candidate_epochs: {candidate_epochs}\ncandidate_results: {candidate_results}\ncandidate_results_mean: {candidate_results_mean.tolist()}')
        with open(log_file, 'a') as f:
            print(
                f'candidate_epochs: {candidate_epochs}\ncandidate_results: {candidate_results}\ncandidate_results_mean: {candidate_results_mean.tolist()}',
                file=f)
